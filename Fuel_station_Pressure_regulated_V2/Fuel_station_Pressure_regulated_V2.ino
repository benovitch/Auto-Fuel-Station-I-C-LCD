...
const int ButtonEnterPin = 9;
const int ButtonEscPin = 10;
...
int lastEnterPinState = 0;
int lastEscPinState = 0;
...
void readBtn()
{
  ...
  ButtonEscState = digitalRead(ButtonEscPin);
  ...
  // Button Enter
  if (ButtonEnterState != lastEnterPinState) {
  ...
  lastEnterPinState = ButtonEnterState;

  // Button Enter 2
  if (ButtonEscState != lastEscPinState) {
    if (ButtonEscState == LOW) {
      btnEsc;
      Serial.println("Button Esc");
    }
    delay(50);
  }
  lastEscPinState = ButtonEscState;
...
void btnUp()
{
  ...
  {
    Serial.println("Mode Fill or Mode Cal");
    menu_pwr++;
